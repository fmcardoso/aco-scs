...
def generate(fragmentsDir, genomeDir, id, sequenceSize = 1000, fragmentMinSize = 35, fragmentMaxSize = 50, coverage = 2):

    random.seed(1234 * id)
    # queremos uma cobertura de 10 vezes pelo menos
    numFragments = math.ceil((sequenceSize / fragmentMinSize) * coverage)
    ...
    for i in range (0, numFragments):
        index = randint(0, sequenceSize - fragmentMaxSize)
